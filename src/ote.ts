...
const intraDayMarketUrl = 'https://www.ote-cr.cz/en/short-term-markets/electricity/intra-day-market/@@chart-data';
...
export class IntraDayMarketLoader implements LoaderPlugin<IntraDayMarketItem> {
  ...
  public async load(date: Date): Promise<IntraDayMarketItem[]> {
    const response:AxiosResponse<IntraDayMarketData,any> = await axios.get(intraDayMarketUrl,
      {
        params: {
...
class IntraDayMarketItemFactory {
  ...
  constructor(data: IntraDayMarketData) {
    this.prices = this.getSeriePoints(data, 'Weighted average of the prices (EUR/MWh)');
  ...
  createAllItems(date: Date): IntraDayMarketItem[] {
    let hours = Array.from(this.prices.keys())
    if (this.prices.size === 0) {
      console.log('no average prices found in the intra-day market data');
    }
    if (this.volumes.size === 0)
    return Array.from(this.prices.keys()).map(
      hour => IntraDayMarketItem.create(date, hour, this.prices.get(hour), this.volumes)
    );
  }
